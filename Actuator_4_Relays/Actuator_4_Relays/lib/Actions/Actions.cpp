...
action_type ActionsClass::decodeActionType(byte *payload) {
	...
	if(relay_on_pulse) {
		debug->println("Relay On Pulse");
		return RELAY_ON_PULSE;
	}
	...
	if(relay2_on_pulse) {
		debug->println("Relay 2 On Pulse");
		return RELAY2_ON_PULSE;
	}
...
void ActionsClass::setAction(byte *payload){
	...
	if(type == RELAY_ON_PULSE || type == RELAY_ON_TIME)  {
		actionStart = millis();
		seconds = 0;
		minutes = 0;
	...
	else if (type == RELAY2_ON_PULSE || type == RELAY2_ON_TIME) {
		actionStart = millis();
		seconds2 = 0;
		minutes2 = 0;
...
void ActionsClass::relayOnPulse(){
	if(!relay && remaining > 0) {
		debug->println("Relay on Pulse begins!");
		relayOn();
	}
...
void ActionsClass::relay2OnPulse(){
	if(!relay && remaining2 > 0) {
		debug->println("Relay 2 on Pulse begins!");
		relay2On();
	}
